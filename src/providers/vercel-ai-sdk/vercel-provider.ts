...
export class VercelAIProvider implements AIProvider {
  ...
  private getProviderOptions() {
    if (this.openaiStoreMode === "force-false") {
      return {
        openai: {
  ...
  private injectProviderOptions(input: Record<string, unknown>): Record<string, unknown> {
    const providerOptions = this.getProviderOptions();
    if (providerOptions) {
      input.providerOptions = providerOptions;
    }
    return input;
  }
  ...
  async ask(prompt: string): Promise<ProviderResult> {
    ...
    const result = await this.agent.generate(
      this.injectProviderOptions({
        prompt,
        onStepFinish: (step: { toolResults: Array<{ output: unknown }> }) => {
  ...
  async askWithSession(
    ...
    const compactionResult = await compactIfNeeded(
      ...
      async summarizePrompt => {
        const r = await this.agent.generate(
          this.injectProviderOptions({ prompt: summarizePrompt }) as never
        );
        return r.text ?? "";
      }
    );
