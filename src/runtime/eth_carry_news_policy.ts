...
import type { NewsImpactSummary, NewsTheme, NewsFlag } from "./news_impact.js";
...
function detectVenue(text: string, venues: readonly ("binance" | "okx")[]): "binance" | "okx" | null {
...
function detectVenueIncidents(
  ...
  for (const item of news) {
    ...
    const isDuplicate = accepted.some(existing => {
      ...
      if (Math.abs(existing.timeMs - candidate.timeMs) > INCIDENT_DEDUP_WINDOW_HOURS * 3_600_000) {
        return false;
      }
