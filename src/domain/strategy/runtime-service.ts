...
type RuntimeSourceManager = Pick<UTAManager, 'resolve'>
...
async function resolveDerivativesData(input: {
  utaManager?: RuntimeSourceManager
  request: RuntimeStrategySnapshotRequest
}): Promise<ResolvedDerivativesData> {
  const { request } = input
  const provenance = createUnavailableStrategyDataProvenance()
  const accountTarget = input.utaManager
    ? resolveSingleCcxtTarget(input.utaManager, request.source)
    : null
...
export async function evaluateRuntimeStrategySnapshotFromSources(input: {
  utaManager?: RuntimeSourceManager
  accountManager?: RuntimeSourceManager
  ...
  if (utaManager && request.source) {
    const targets = utaManager.resolve(request.source)
    if (targets.length === 1) {
      ...
      if (resolvedEquity == null && account?.netLiquidation != null) {
        resolvedEquity = Number(account.netLiquidation)
        derivatives.dataProvenance.equity = makeSourceStatus(
          'account-broker',
          'resolved',
