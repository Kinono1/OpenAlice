...
function toGitOrder(contract: Contract, req: CryptoPlaceOrderRequest): Order {
  ...
  if (typeof req.usd_size === 'number' && req.usd_size > 0) {
    order.cashQty = new Decimal(String(req.usd_size))
  }
  if (req.type === 'limit' && typeof req.price === 'number' && req.price > 0) {
    order.lmtPrice = new Decimal(String(req.price))
  }
...
function toCryptoPosition(position: Position): CryptoPosition {
  return {
    ...
    entryPrice: Number(position.avgCost),
    leverage: 1,
    margin: 0,
    liquidationPrice: 0,
    markPrice: Number(position.marketPrice),
    unrealizedPnL: Number(position.unrealizedPnL),
    positionValue: Number(position.marketValue),
  }
}
...
function toCryptoTicker(symbol: string, quote: Quote): CryptoTicker {
  return {
    ...
    high: Number(quote.high ?? quote.last),
    low: Number(quote.low ?? quote.last),
    ...
    case 'placeOrder': {
      ...
      const size =
        op.order.totalQuantity && !op.order.totalQuantity.equals(UNSET_DECIMAL)
          ...
          : undefined
      const usd_size =
        op.order.cashQty && !op.order.cashQty.equals(UNSET_DECIMAL)
          ? op.order.cashQty.toNumber()
          : undefined
      const price =
        op.order.lmtPrice && !op.order.lmtPrice.equals(UNSET_DECIMAL)
          ? op.order.lmtPrice.toNumber()
          : undefined
...
export class CcxtTradingEngineAdapter implements ICryptoTradingEngine {
  ...
  async getAccount(): Promise<CryptoAccountInfo> {
    ...
    return {
      balance: Number(account.totalCashValue),
      totalMargin: Number(account.initMarginReq ?? 0),
      unrealizedPnL: Number(account.unrealizedPnL),
      equity: Number(account.netLiquidation),
      realizedPnL: Number(account.realizedPnL ?? 0),
      totalPnL: Number(account.realizedPnL ?? 0) + Number(account.unrealizedPnL),
    }
  }
...
export async function createCcxtExecutionBridge(input: {
  ...
  const utaManager = input.utaManager ?? input.accountManager
  ...
  const dispatcher = createCryptoOperationDispatcher(engine, {
    ...
    preparePlaceOrder: async ({ request, expectedPrice }) => {
      ...
      if (!utaManager || !input.cryptoClient) {
        if (freeze?.active) {
