...
import { runStrategyBacktest } from "../extension/strategy-tools/backtest.js";
import type { BacktestCostModel, BacktestMetrics } from "../extension/strategy-tools/backtest.js";
import type { StrategyName, StrategyParams } from "../extension/strategy-tools/types.js";
...
export function runStrategyWalkForward(input: StrategyWfoInput): WfoResult<StrategyParams> {
  ...
  for (let windowIndex = 0; windowIndex < windows.length; windowIndex++) {
    ...
    for (let i = 1; i < candidates.length; i++) {
      ...
      const candidateMetrics = toWindowMetrics(
        runStrategyBacktest({
          strategy: input.strategy,
          candles: trainCandles,
      ...
      const betterSharpe = candidateMetrics.sharpe > selectedInSample.sharpe;
      const tieBreakByReturn =
        candidateMetrics.sharpe === selectedInSample.sharpe &&
        candidateMetrics.totalReturnPct > selectedInSample.totalReturnPct;

      if (betterSharpe || tieBreakByReturn) {
        selected = candidate;
        selectedInSample = candidateMetrics;
      }
    }

    const oosMetrics = toWindowMetrics(
      runStrategyBacktest({
        strategy: input.strategy,
        candles: testCandles,
        params: selected.params,
...
function evaluateWindowGate(
  ...
  const degradationRate = (inSample.sharpe - outOfSample.sharpe) / Math.abs(inSample.sharpe);
...
function resolveWfoConfig(config: WfoConfig): Required<WfoConfig> {
  const trainBars = toPositiveInt(config.trainBars ?? DEFAULT_WFO_CONFIG.trainBars, "trainBars");
  const testBars = toPositiveInt(config.testBars ?? DEFAULT_WFO_CONFIG.testBars, "testBars");
  const stepBars = toPositiveInt(config.stepBars ?? testBars, "stepBars");
  const degradationThreshold = toFinite(config.degradationThreshold ?? DEFAULT_WFO_CONFIG.degradationThreshold, "degradationThreshold");
  const minTradesPerWindow = toNonNegativeInt(config.minTradesPerWindow ?? DEFAULT_WFO_CONFIG.minTradesPerWindow, "minTradesPerWindow");
