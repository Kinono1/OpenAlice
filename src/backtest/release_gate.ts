import type { WfoResult } from "./wfo.js";
...
export interface ReleaseGateInput<TParams = unknown> {
  wfo?: Pick<WfoResult<TParams>, "overallPassed" | "failedWindows" | "windows">;
...
function evaluateRegimeShiftCheck(
  ...
  if (regimeShift.severity === "high" && regimeShift.triggered) {
    return {
      name: "regime_shift",
      status: "fail",
      summary: regimeShift.reason ?? "Regime-shift high severity detected.",
      metrics: {
  ...
  if (regimeShift.severity === "watch" && regimeShift.triggered) {
    return {
      name: "regime_shift",
      status: "warn",
      summary: regimeShift.reason ?? "Regime-shift watch severity detected.",
      metrics: {
        triggered: regimeShift.triggered,
        severity: regimeShift.severity,
      },
