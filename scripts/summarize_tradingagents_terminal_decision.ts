...
async function main(): Promise<void> {
  ...
  await appendExecutionJournal(
    {
      ...
      status: 'started',
      inputs: { diagnoses: args.diagnoses.map((path) => resolve(path)) },
  ...
  try {
    ...
    const payload = summarizeTradingAgentsTerminalDecision({
...
function parseArgs(argv: string[]): CliArgs {
  ...
  if (!output || diagnoses.length < 1) {
    throw new Error('--output and at least one --diagnosis <path> are required.')
  }
  return {
    diagnoses,
    output,
