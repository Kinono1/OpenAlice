...
def validate_reason_codes_file(payload: dict[str, Any]) -> tuple[set[str], dict[str, str]]:
...
def main() -> int:
    ...
    evidence_pack_path = (
        Path(args.evidence_pack) if args.evidence_pack else packet_dir / "evidence_pack.json"
    )
    reason_codes_path = Path(args.reason_codes)
    freeze_manifest_path = Path(args.freeze_manifest)
    output_path = (
        Path(args.output) if args.output else packet_dir / "verdict.json"
    )
    ...
    try:
        ...
        canonical_codes, deprecated_aliases = validate_reason_codes_file(reason_codes_payload)
        ...
        for required_code in (
            ...
            if required_code not in canonical_codes:
                raise ValueError(f"reason code missing in canonical list: {required_code}")
        ...
        if release_status_path and release_status_path.exists():
            ...
            if expires_at and datetime.now(timezone.utc) > expires_at.astimezone(timezone.utc):
        ...
        checks.append(
            {
                ...
                "passed": "HARD_RELEASE_GATE_BLOCKED" not in reasons_hard
        ...
        metric_failed = [item for item in threshold_results if not item["passed"]]
        if metric_missing:
