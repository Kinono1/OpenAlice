/**
 * Accumulate 1-minute live data from OKX - run every 5 minutes.
 *
 * Flow:
 *   1. Fetch latest 300 1m candles from OKX for each asset
 *   2. Merge with existing CSV (dedup by timestamp)
 *   3. Save updated CSV
 *
 * Usage: npx tsx scripts/accumulate_1m_data.ts
 */
...
const ASSETS = defaultMarketDataUniverseAssets('1m').map(asset => ({
  base: asset.base,
  instId: asset.okxInstId,
  symbol: asset.storageSymbol,
  file: paperSymbolToCsvFile(asset.paperSymbol, '1m'),
}))
...
const DEFAULT_FETCH_ATTEMPTS = parsePositiveIntegerEnv('OPENALICE_OKX_1M_FETCH_ATTEMPTS', 5)
const DEFAULT_FETCH_RETRY_DELAY_MS = parsePositiveIntegerEnv('OPENALICE_OKX_1M_FETCH_RETRY_DELAY_MS', 1500)
...
export function rowsToCSV(header: string, rows: Map<number, CandleRow>, symbol: string, exchange: string): string {
  ...
  for (const r of sorted) {
    lines.push([r.timestamp, new Date(r.timestamp).toISOString(), r.open, r.high, r.low, r.close, r.volume, symbol, '1m', exchange].join(','))
  }
  return lines.join('\n')
}
...
async function main() {
  ...
  const outDir = join(import.meta.dirname ?? '.', '..', 'data', 'market', 'live_1m')
  await mkdir(outDir, { recursive: true })
  const generatedAt = new Date().toISOString()
  const jobId = 'okx_public_ohlcv_1m_collector'
  const collectionRunId = buildCollectorRunId({ jobId, generatedAt, timeframe: '1m' })
  ...
  for (const asset of assets) {
    ...
    try {
      ...
      const latest = await fetchCandlesWithRetry(fetchLiveCandles, asset.instId, '1m', 300)
  ...
  console.log(`Total new 1m bars: ${totalNew}`)
