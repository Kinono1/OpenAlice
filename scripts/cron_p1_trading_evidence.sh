...
LOCK_DIR="$ARTIFACT_ROOT/locks/p1_trading_evidence.lock"
LOG_DIR="$LOG_ROOT/cron"
LOG_FILE="$LOG_DIR/p1_trading_evidence.log"
INDEX_PATH="$ARTIFACT_ROOT/p1_trading_evidence/p1_trading_evidence.index.latest.json"
META_LABEL_READINESS_PATH="$ARTIFACT_ROOT/meta_labeling_shadow_readiness.latest.json"
NOTIFICATION_PATH="$ARTIFACT_ROOT/p1_trading_evidence_notification.json"
...

{
  ...
  cd "$REPO_ROOT"
  ./node_modules/.bin/tsx scripts/build_p1_trading_evidence.ts --json true --outputDir "$ARTIFACT_ROOT/p1_trading_evidence" > "$INDEX_PATH.stdout"
