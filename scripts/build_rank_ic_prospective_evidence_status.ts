...
export interface RankIcProspectiveEvidenceStatusReport {
  schemaVersion: 1
  generatedAt: string
  researchOnly: true
...
const DEFAULT_OUTPUT_PATH = 'data/research/rank_ic_prospective_evidence_status.live_accumulated_fwd72_median_filter.latest.json'
const DEFAULT_MIN_CLOSED_OUTCOMES = 100
const DEFAULT_MIN_NON_OVERLAPPING_WINDOWS = 3

async function main(): Promise<void> {
  const args = parseRankIcProspectiveEvidenceStatusArgs(process.argv.slice(2))
  const startedAt = new Date()
  const report = await runRankIcProspectiveEvidenceStatus(args)
  ...
  if (args.outputPath) {
    await writeEvidenceManifestForArtifact({
      job: 'rank_ic_prospective_evidence_status',
...
export function parseRankIcProspectiveEvidenceStatusArgs(argv: string[]): CliArgs {
...
export async function runRankIcProspectiveEvidenceStatus(
  args: CliArgs,
): Promise<RankIcProspectiveEvidenceStatusReport> {
...
export function buildRankIcProspectiveEvidenceStatusReport(input: {
  ledgerPath: string
  ...
  generatedAt?: string
}): RankIcProspectiveEvidenceStatusReport {
  const generatedAt = input.generatedAt ?? new Date().toISOString()
  const openById = new Map<string, RankIcProspectiveObservationEvent>()
  ...
  const status: RankIcProspectiveEvidenceStatusReport['status'] = !input.ledgerExists
    ? 'blocked_no_ledger'
...
function renderConsoleSummary(report: RankIcProspectiveEvidenceStatusReport): string {
  return [
    `rank ic prospective evidence status: status=${report.status}, open=${report.counts.openEvents}, closed=${report.counts.closedEvents}`,
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  main().catch(error => {
    console.error('build_rank_ic_prospective_evidence_status failed:', error)
    process.exit(1)
  })
}
