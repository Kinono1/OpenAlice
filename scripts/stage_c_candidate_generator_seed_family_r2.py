#!/usr/bin/env python3
"""Generate Round 2 candidates for the single vol-gated breakout seed family."""
...
DEFAULT_OUTPUT = "docs/research/stage_c_strategy_candidates.seed_family.r2.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate round-2 seed-family candidates.")
...
def feature_file(feature_root: Path, inst_id: str) -> Path:
    path = feature_root / f"okx_inst_id={inst_id}" / "data.csv"
    if not path.exists():
        raise FileNotFoundError(f"Missing target table for {inst_id}: {path}")
    return path
...
def generate_candidates(stats: Dict[str, float]) -> List[Dict[str, Any]]:
    vol_thresholds = [1.05, 1.15, 1.25]
    breakout_sets = [(12, 6), (20, 10), (30, 15)]
    cooldowns = [0, 12]
    ...
    for cooldown in cooldowns:
        for vol_trigger in vol_thresholds:
            for breakout_period, breakout_exit in breakout_sets:
                candidates.append(
                    {
                        "strategyId": f"STC_SEED_VB_R2_{idx}",
                        "strategyName": f"seed_r2_vol_breakout_{breakout_period}_{breakout_exit}_{vol_trigger:.2f}_cd{cooldown}",
...
def main() -> int:
    ...
    payload["stageCSeedFamily"] = {
        "schemaVersion": "stage_c_seed_family_round2.v1",
        "generatedAt": utc_iso(),
