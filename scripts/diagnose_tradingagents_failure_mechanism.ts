...
async function main(): Promise<void> {
  ...
  try {
    ...
    const diagnosis = diagnoseTradingAgentsFailureMechanism({
      ...
      sourcePreRegisteredConfig: args.preRegisteredConfig ? resolve(args.preRegisteredConfig) : null,
    })
...
function parseArgs(argv: string[]): CliArgs {
  ...
  if (!output || !validationRuns || !routeMatrix) {
    throw new Error('--validation-runs, --route-matrix, and --output are required.')
  }
  return {
    validationRuns,
    routeMatrix,
