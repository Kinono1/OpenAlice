...
function parseGateProfile(raw: string | undefined): GateProfile {
  ...
  if (value === "stage1" || value === "stage2" || value === "hard") {
    return value;
  }
  return "hard";
}
...
function parseRegimeScheme(raw: string | undefined): RegimeScheme {
  ...
  if (value === "rule_v1" || value === "kmeans_v1") {
    return value;
  }
  return "rule_v1";
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  const objectiveModeRaw = (raw.get("objectiveMode") ?? "auto")
    .trim()
    .toLowerCase();
  const objectiveMode: "auto" | "max" | "min" =
    objectiveModeRaw === "max" || objectiveModeRaw === "min"
      ? objectiveModeRaw
      : "auto";
  const gateProfile = parseGateProfile(
    raw.get("gateProfile") ?? raw.get("gate-profile")
  );
  const partitionMode = parsePartitionMode(
    raw.get("partitionMode") ?? raw.get("partition-mode")
  );
  const regimeScheme = parseRegimeScheme(
    raw.get("regimeScheme") ?? raw.get("regime-scheme")
  );
  ...
  return {
    ...
    skipInsufficientSymbols: parseBoolArg(
      raw.get("skipInsufficientSymbols"),
      true
    ),
    gateProfile,
    partitionMode,
...
function objectiveMode(
  ...
  if (metric === "maePct" || metric === "rmsePct") {
    return "min";
  }
  return "max";
}
...
function resolveWalkForwardWindows(
  ...
  if (trainBars + testBars <= availableBars && trainBars >= minTrainBars) {
    return {
      trainBars,
      testBars,
      stepBars: Math.max(1, Math.min(stepBars, testBars)),
    };
  }

  // Adaptive fallback for shorter histories.
  ...
  if (
    trainBars + testBars > availableBars ||
    trainBars < minTrainBars ||
    testBars < minTestBars
  ) {
    return null;
  }
...
function splitSymbolKey(symbolKey: string): {
  baseSymbol: string;
  sourceBucket: string | null;
} {
  const marker = "::";
...
function evaluateCompletionScore(input: ScoreInput): ScoreResult {
  ...
  const components: Array<{ name: string; weight: number; value: number }> = [
    ...
    {
      name: "ml_positive_lift_ratio",
      weight: 0.13,
      value: input.positiveLiftRatio,
    },
    ...
    {
      name: "significance_pass_ratio",
      weight: 0.1,
      value: input.significancePassRatio,
    },
  ...
  if (input.meanPbo > input.maxMeanPbo) {
    hardGateFailures.push(
      `meanPbo=${input.meanPbo.toFixed(3)} > ${input.maxMeanPbo.toFixed(3)}`
    );
  }
  ...
  const hardGateApplied =
    input.enforceSignificanceHardGate && hardGateFailures.length > 0;
  let score = rawScore;
  if (hardGateApplied) {
...
async function main(): Promise<void> {
  ...
  const maxScoreWhenHardGateFails = clamp(
    args.maxScoreWhenHardGateFails,
    0,
    100
  );
  const cwd = resolve(".");
  ...
  if (!(await fileExists(cleanSummaryPath))) {
    throw new Error(`Missing clean summary: ${cleanSummaryPath}`);
  }
  if (!(await fileExists(retrainSummaryPath))) {
    throw new Error(`Missing retrain summary: ${retrainSummaryPath}`);
  }
  if (!(await fileExists(retrainResultsPath))) {
    throw new Error(`Missing retrain results: ${retrainResultsPath}`);
  }

  const cleanSummary = JSON.parse(
    await readFile(cleanSummaryPath, "utf-8")
  ) as {
    symbols: CleanSummarySymbol[];
  };
  const retrainSummary = JSON.parse(
    await readFile(retrainSummaryPath, "utf-8")
  ) as Record<string, any>;
  const retrainResults = parseJsonArray<RetrainResultRow>(
    await readFile(retrainResultsPath, "utf-8"),
    "retrain results"
  );

  const objectiveMetric = args.objectiveMetric;
  const mode = objectiveMode(objectiveMetric, args.objectiveMode);
  ...
  const ranked = [...retrainResults]
    .map(row => ({
      row,
      score: getMetricScore(row, objectiveMetric),
    ...
    .sort((a, b) => {
      ...
      return mode === "max" ? bScore - aScore : aScore - bScore;
    });
  ...
  const selectedRaw = ranked.slice(0, args.topSymbols).map(item => {
    ...
    const latestRegimeLabel =
      meta?.latestRegimeLabel ?? getCurrentRegime(item.row);
    const partitionId =
      meta?.partitionId ??
      resolvePartitionId(args.partitionMode, sourceBucket, latestRegimeLabel);
    return {
  ...
  const selected = selectedRaw.filter(entry => {
    if (!args.skipInsufficientSymbols) {
      return true;
    }
    const availableBars = Math.min(
      args.lookbackBars,
      Math.max(0, Number(entry.rows) || 0)
    );
  ...
  for (const entry of selected) {
    for (const strategy of args.strategies) {
      ...
      console.log(
        `[${runIndex}/${totalRuns}] validate strategy=${strategy} symbol=${entry.symbol}`
      );
      const cmdArgs = [
        "--import",
        "tsx",
      ...
      const proc = await runProcess(process.execPath, cmdArgs, cwd);
      let report: Record<string, any> | undefined;
      let error: string | undefined;

      if (await fileExists(outputPath)) {
        try {
          report = JSON.parse(await readFile(outputPath, "utf-8")) as Record<
            string,
            any
          >;
        } catch (err) {
      ...
      // run_validation_pipeline uses exit code 2 when release gate blocks paper trading.
  ...
  const paperPassCount = successRuns.filter(
    run => run.report?.releaseGate?.allowPaperTrading === true
  ).length;
  const livePassCount = successRuns.filter(
    run => run.report?.releaseGate?.allowLiveTrading === true
  ).length;
  const paperPassRatio = successRuns.length
    ? paperPassCount / successRuns.length
    : 0;
  const livePassRatio = successRuns.length
    ? livePassCount / successRuns.length
    : 0;
  const significancePassCount = successRuns.filter(
    run => run.report?.significance?.passed === true
  ).length;
  const significancePassRatio = successRuns.length
    ? significancePassCount / successRuns.length
    : 0;
  const meanPbo = mean(
    successRuns
  ...
  const avgReturnPct = mean(
    successRuns
      .map(run =>
        normalizePct(Number(run.report?.selectedMetrics?.totalReturnPct))
      )
      .filter((value): value is number => value != null)
  );
  const avgMaxDrawdownPct = mean(
    successRuns
      .map(run =>
        normalizePct(
          Number(run.report?.selectedMetrics?.maxDrawdownPct),
          0,
          100
        )
      )
  ...
  for (const strategy of args.strategies) {
    ...
    strategyBreakdown[strategy] = {
      runs: subset.length,
      paperPass: subset.filter(
        run => run.report?.releaseGate?.allowPaperTrading
      ).length,
      livePass: subset.filter(run => run.report?.releaseGate?.allowLiveTrading)
        .length,
      avgSharpe: mean(
        subset
          .map(run =>
            normalizeSharpe(Number(run.report?.selectedMetrics?.sharpe))
          )
          .filter((value): value is number => value != null)
      ),
      avgReturnPct: mean(
  ...
  const positiveLiftRatio = clamp(
    Number(retrainSummary.positiveLiftRatio ?? 0),
    0,
    1
  );
  ...
  const completionScore = overallScore.score;
  const rawCompletionScore = overallScore.rawScore;
  const readiness = overallScore.readiness;
  const hardGateFailures = overallScore.hardGateFailures;
  const components = overallScore.components;

  const partitionSymbolMap = new Map<string, Set<string>>();
  for (const entry of selected) {
    const pid = entry.partitionId ?? "all";
    if (!partitionSymbolMap.has(pid)) {
      partitionSymbolMap.set(pid, new Set<string>());
    }
    partitionSymbolMap.get(pid)!.add(entry.symbol);
  }
  const metricsByPartition = Array.from(partitionSymbolMap.entries())
    .map(([partitionId, symbolSet]) => {
      const symbols = [...symbolSet];
      const symbolLookup = new Set(symbols);
      const runSubset = successRuns.filter(run => symbolLookup.has(run.symbol));
      const partitionPaperPassRatio = runSubset.length
        ? runSubset.filter(run => run.report?.releaseGate?.allowPaperTrading).length /
          runSubset.length
        : 0;
      const partitionLivePassRatio = runSubset.length
        ? runSubset.filter(run => run.report?.releaseGate?.allowLiveTrading).length /
          runSubset.length
        : 0;
      const partitionSignificancePassRatio = runSubset.length
        ? runSubset.filter(run => run.report?.significance?.passed === true).length /
          runSubset.length
        : 0;
      const partitionMeanPbo = mean(
        runSubset
          .map(run => Number(run.report?.significance?.pbo))
          .filter(value => Number.isFinite(value))
          .map(value => clamp(value, 0, 1))
      );
      const partitionMeanDsrProbability = mean(
        runSubset
          .map(run => Number(run.report?.significance?.dsrProbability))
          .filter(value => Number.isFinite(value))
          .map(value => clamp(value, 0, 1))
      );
      const partitionAvgSharpe = mean(
        runSubset
          .map(run => normalizeSharpe(Number(run.report?.selectedMetrics?.sharpe)))
          .filter((value): value is number => value != null)
      );
      const partitionLifts = symbols
        .map(symbol => Number(retrainBySymbol.get(symbol)?.metrics?.accuracyLift))
        .filter(value => Number.isFinite(value));
      const partitionMeanAccuracyLift = partitionLifts.length
        ? mean(partitionLifts)
        : 0;
      const partitionPositiveLiftRatio = partitionLifts.length
        ? partitionLifts.filter(value => value > 0).length / partitionLifts.length
        : 0;
      const partitionScore = evaluateCompletionScore({
        meanAccuracyLift: partitionMeanAccuracyLift,
        positiveLiftRatio: partitionPositiveLiftRatio,
        paperPassRatio: partitionPaperPassRatio,
        livePassRatio: partitionLivePassRatio,
        significancePassRatio: partitionSignificancePassRatio,
        meanPbo: partitionMeanPbo,
        meanDsrProbability: partitionMeanDsrProbability,
        avgSharpe: partitionAvgSharpe,
        runUnitTests: false,
      ...
      return {
        ...
        meanDsrProbability: partitionMeanDsrProbability,
        avgSharpe: partitionAvgSharpe,
        score: Number(partitionScore.score.toFixed(2)),
        readiness: partitionScore.readiness,
        hardGateApplied: partitionScore.hardGateApplied,
      };
    })
  ...
  if (meanAccuracyLift <= 0) {
    recommendations.push(
      "ML objective has non-positive mean lift; improve feature/model objective alignment."
    );
  }
  if (positiveLiftRatio < 0.5) {
    recommendations.push(
      "Positive-lift symbol ratio is low; add regime filters and per-symbol model routing."
    );
  }
  if (paperPassRatio < 0.6) {
    recommendations.push(
      "Strategy release-gate pass ratio is low; retune params with stricter candidate selection."
    );
  }
  if (significancePassRatio < minSignificancePassRatio) {
    recommendations.push(
      `Significance pass ratio is below threshold (${significancePassRatio.toFixed(3)} < ${minSignificancePassRatio.toFixed(3)}).`
    );
  }
  if (meanPbo > maxMeanPbo) {
    recommendations.push(
      `Mean PBO is too high (${meanPbo.toFixed(3)} > ${maxMeanPbo.toFixed(3)}), indicating likely overfitting.`
    );
  }
  if (meanDsrProbability < minMeanDsrProbability) {
    recommendations.push(
      `Mean DSR probability is too low (${meanDsrProbability.toFixed(3)} < ${minMeanDsrProbability.toFixed(3)}).`
    );
  }
  if (args.enforceSignificanceHardGate && hardGateFailures.length > 0) {
    recommendations.push(
      `Significance hard gate active; readiness forced to not_ready. Reasons: ${hardGateFailures.join("; ")}`
    );
  }
  if (args.runUnitTests && !unitTests.passed) {
    recommendations.push(
      "Unit tests are failing; stabilize code quality gate before deployment."
    );
  }
  if (!recommendations.length) {
    recommendations.push(
      "Current setup passes baseline completion gates; continue paper trading and drift monitoring."
    );
  }

  const selectedModelCounts = selected.reduce(
    (acc, row) => {
  ...
  const report = {
    ...
    input: {
      ...
      skipInsufficientSymbols: args.skipInsufficientSymbols,
      significanceGate: {
        minSignificancePassRatio,
        maxMeanPbo,
        minMeanDsrProbability,
    ...
    ml: {
      ...
      meanBaselineDirectionAccuracy: Number(
        retrainSummary.meanBaselineDirectionAccuracy ?? 0
      ),
      meanAccuracyLift,
      positiveLiftRatio,
      selectedModelCounts,
    ...
    unitTests,
    completion: {
      score: Number(completionScore.toFixed(2)),
      rawScore: Number(rawCompletionScore.toFixed(2)),
      readiness,
  ...
  // Write aggregated release gate status so the runtime can find it
  ...
  const allowPaper = readiness === "ready_for_paper" || readiness === "candidate_needs_iteration";
  const allowLive = readiness === "ready_for_paper";
  ...
  await writeReleaseGateStatus(
    {
      ...
      hardFail: report.completion.hardGateApplied,
