...
SCRIPT_PATH = REPO_ROOT / "scripts" / "stage_c_round4_candidate_generator.py"
...
class TestStageCRound4CandidateGenerator(unittest.TestCase):
    def test_generator_produces_three_candidates_per_mapping(self) -> None:
        with tempfile.TemporaryDirectory(prefix="openalice-stagec-r4-gen-") as tmp:
            root = Path(tmp)
                    ...
                    values.append(
                        ",".join(
                            [
                                ...
                                "1m",
                                f"{100 + i * scale:.6f}",
                                f"{0.002 * scale * ((i % 5) + 1):.6f}",
                                f"{0.004 * scale * ((i % 7) + 1):.6f}",
                                f"{0.001 * scale * ((i % 3) + 1):.6f}",
                                f"{0.003 * scale * ((i % 4) + 1):.6f}",
                            ]
                        )
                    )
                return "\n".join(values) + "\n"
            ...
            expectations = {
                "no_trade": "volNoTradeFilter",
                "breakout": "volBreakout",
                "trend": "volTrend",
            }
            for mapping, strategy in expectations.items():
                output_path = root / f"{mapping}.candidates.v1.json"
                run = subprocess.run(
                    [
                        sys.executable,
                        str(SCRIPT_PATH),
                ...
                self.assertEqual({item["strategy"] for item in payload["candidates"]}, {strategy})
                self.assertEqual(payload["stageCRound4Mapping"]["mappingKey"], mapping)
