...
SCRIPT_PATH = REPO_ROOT / "scripts" / "stage_c_candidate_generator_seed_family_r2.py"
...
class TestStageCCandidateGeneratorSeedFamilyR2(unittest.TestCase):
    def test_generator_produces_eighteen_candidates(self) -> None:
        with tempfile.TemporaryDirectory(prefix="openalice-stagec-seed-r2-") as tmp:
            root = Path(tmp)
            base_path = root / "base_candidates.v1.json"
            feature_root = root / "target_tables"
            output_path = root / "seed_r2_candidates.v1.json"
            ...
            base_payload = {
                ...
                "thresholds": {"meanPboMax": 0.2, "meanDsrProbabilityMin": 0.5, "fdrQMax": 0.1},
            ...
            cooldowns = sorted({row["params"]["volCooldownBars"] for row in candidates})
            self.assertEqual(cooldowns, [0, 12])
