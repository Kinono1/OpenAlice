...
- Checkpoint store is integrated into `scripts/run_route_matrix.ts`; other long-running scripts should still be integrated one at a time with separate tests.
...
- Integrate `RuntimeCheckpointStore` into the next long-running script, such as `scripts/run_phaseb_trial_sweep.ts`, with separate `--resume`, `--fresh`, and optional `--run-id` tests.
